...
function updateMouthEyes() {
  if (username.value.length > 0) {
    if (username.value.indexOf("@") > 0 && username.value.indexOf("@") < username.value.length - 1) {
  ...
  let movePos = username.value.length > 30 ? 13.33 : username.value.length / 2.25;
...
username.addEventListener("focus", updateMouthEyes);
username.addEventListener("input", updateMouthEyes);
